...
df = pd.read_excel(DATA_FILE, sheet_name='增长代理资产', index_col=0, parse_dates=True)
df.index.name = 'Date'
...
# --- 按照要求修改为中文命名 ---
# 将结果保存为 CSV 文件
csv_filename = SCRIPT_DIR / '增长因子组合净值数据.csv'
...
plt.grid(True)
plt.tight_layout()
plt.savefig(SCRIPT_DIR / '增长因子组合净值走势图.png')
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(SCRIPT_DIR / '增长因子组合净值同比走势图.png')
plt.close()
