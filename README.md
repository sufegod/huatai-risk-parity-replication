...
- `策略复现与回测/`: 风险平价策略回测入口、历史策略版本、回测图表和回测指标。不同回测版本代码保留在 `策略代码/` 或 `历史策略版本/` 中完整展示；`回测指标/` 下按 `净值/`、`指标/`、`仓位明细/`、`对比结果/` 分类。
- `策略复现与回测/每日更新策略/`: 每日策略报告脚本和按用途分类的输出目录。
- `策略复现与回测/策略测试代码/`: 参数测试、调仓频率测试等实验脚本。
- `策略复现与回测/策略测试结果/`: 参数测试、品种替换、资产删除等策略实验输出。
- `策略复现与回测/品种测试流程提示词.md`: 用于在其他模型中稳定执行品种新增、删除、替换测试的可复制提示词。
