...
def main():
    ...
    df = pd.read_excel(FILE_PATH_ETF, sheet_name=SHEET_NAME, index_col=0, parse_dates=True)
    ...
    asset_weight_records = []  # 【新增】记录底层的 ETF 权重

    print("执行全天候增强策略 (宏观动量择时 + 动态组合平价)...")
    ...
    for i in range(len(month_ends) - 1):
        ...
        if rebalance_date not in df_mom_monthly.index or pd.isna(df_mom_monthly.loc[rebalance_date, '增长预期动量']):
            continue

        lookback_start = rebalance_date - pd.DateOffset(months=12)

        g_mom = df_mom_monthly.loc[rebalance_date, '增长预期动量']
        i_mom = df_mom_monthly.loc[rebalance_date, '通胀预期动量']

        selected_growth_quad = '增长超预期' if g_mom > 0 else '增长不及预期'
        selected_inflation_quad = '通胀超预期' if i_mom > 0 else '通胀不及预期'
        active_quadrants = [selected_growth_quad, selected_inflation_quad]

        historical_quadrants = df_quadrants.loc[lookback_start:rebalance_date, active_quadrants]
        if len(historical_quadrants) < 150:
            continue

        cov_matrix = calculate_ewma_semi_cov(historical_quadrants, decay=EWMA_DECAY)
        active_weights = get_risk_parity_weights(cov_matrix)

        current_q_weights = {q: 0.0 for q in quadrant_names}
        current_q_weights[active_quadrants[0]] += active_weights[0]
        current_q_weights[active_quadrants[1]] += active_weights[1]
        ...
        for idx, q_name in enumerate(active_quadrants):
            q_weight = active_weights[idx]
    ...
    if first_trade_date is None:
        raise ValueError("数据不足以生成调仓记录，请检查动量数据日期与ETF数据日期是否重合！")
    ...
    ax1.plot(nav_strategy.index, nav_strategy, label='增强全天候策略 (动量择时)', color='red', linewidth=2)
    ...
    ax1.set_title('策略与基准累计净值走势', fontsize=15)
    ...
    ax2.set_title('底层四大类资产动态轮动权重 (月度)', fontsize=15)
    ax2.set_ylim(0, 1)
    ...
    ax3.set_title('全天候增强：宏观四象限动态轮动权重 (月度)', fontsize=15)
    ax3.set_ylim(0, 1)
