...
def calculate_metrics(ret_series, rf_series, margin_series=None):
    ...
    win_rate = (ret_series.resample('ME').apply(lambda x: (1 + x).prod() - 1) > 0).mean()
...
# ================= 主流程 =================
def main():
    print(f"正在执行回测框架 v{VERSION} (包含逻辑严密的保证金与现金管理)...")
    ...
    metrics = []
    for asset in assets:
        m = calculate_metrics(df_etf.loc[first_date:, asset], repo_net_yield.loc[first_date:])
        m['组合/资产'] = asset
        metrics.append(m)

    for s in strats:
        m = calculate_metrics(ret_dfs[s].loc[first_date:], repo_net_yield.loc[first_date:],
                              margin_dfs[s].loc[first_date:])
        m['组合/资产'] = s
        metrics.append(m)
