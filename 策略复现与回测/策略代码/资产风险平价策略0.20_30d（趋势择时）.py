...
# 趋势择时（实验 v0.20）：对每个风险平价资产用过去 N 个交易日累计收益做二元过滤
# 趋势向上(累计收益>0)保留并按原风险平价比例分配；趋势向下(<=0)剔除，剩余权重重分配给定向品种
TREND_ENABLED = True
TREND_LOOKBACK = 30  # 约1.5个月（30个交易日）
...
# ================= 主流程 =================
def main():
    print(f"正在执行回测框架 v{VERSION}（IC替换IM，日频调仓）...")
    ...
    # 预计算各风险平价资产过去 TREND_LOOKBACK 日累计收益（趋势信号，用于择时过滤）
    df_mom = df_weight_all[risk_parity_assets].rolling(TREND_LOOKBACK).sum() if TREND_ENABLED else None
    listing_dates = {
    ...
    for i in range(len(observation_dates) - 1):
        ...
        if TREND_ENABLED and df_mom is not None:
            mom_row = df_mom.loc[rebalance_date, eligible_rp_assets]
            trend_mask = (mom_row > 0).astype(float).values
            rp_filtered = rp_active * trend_mask
            if rp_filtered.sum() > 1e-12:
                rp_filtered = rp_filtered / rp_filtered.sum()
            else:
                # 全部品种趋势向下：退化为原风险平价权重，避免完全空仓
                rp_filtered = rp_active
            target.loc[eligible_rp_assets] = rp_filtered * remaining_weight
        else:
