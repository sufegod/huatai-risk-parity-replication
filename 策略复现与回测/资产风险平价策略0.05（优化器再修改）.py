...
# ================= 风险平价优化函数 (v0.05 凸优化终极重构) =================
def risk_parity_convex_objective(x, cov_matrix):
    """
    严格凸优化目标函数：最小化 0.5 * x^T * Cov * x - (1/N) * sum(ln(x))
    该函数有且仅有一个全局最优解，彻底消灭权重突变。
    """
    n = len(x)
    # 投资组合未归一化的方差
    port_var = 0.5 * np.dot(x.T, np.dot(cov_matrix, x))
    # 对数惩罚项，迫使每个资产都必须被分配权重
...
def risk_parity_convex_jacobian(x, cov_matrix):
    """
    目标函数的梯度（一阶导数），提供给求解器能成百倍提高寻优速度和精度
    """
    n = len(x)
...
def get_risk_parity_weights(cov_matrix):
    ...
    # 初始化：赋予所有资产均等的正数（因为对数函数域为 x>0）
    x0 = np.ones(n_assets)

    # 设定边界：权重必须大于0，这里用 1e-8 防止对数爆炸
    bounds = [(1e-8, None)] * n_assets

    # 使用 L-BFGS-B 算法，配合梯度 Jacobian，求解极度稳定
    ...
    # 解出的 x 是未归一化的权重，将其加总归一化即可得到完美平滑的真实权重
    x_opt = result.x
