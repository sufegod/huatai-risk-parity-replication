...
def risk_parity_objective(w, cov_matrix):
    ...
    # 【核心修复1】将目标函数的平方误差放大 1e9 倍，
    # 避免由于数值量级过小导致优化器误判已收敛而提前退出
    return np.sum((risk_contribution - target_rc) ** 2) * 1e9
...
def get_risk_parity_weights(cov_matrix):
    ...
    # 【核心修复2】显式设置优化器选项：进一步压低停机容差(ftol)至 1e-10，并调高最大迭代次数
