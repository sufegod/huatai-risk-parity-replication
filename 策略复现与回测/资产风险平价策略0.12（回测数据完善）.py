...
def calculate_metrics(ret_series, rf_series, margin_series=None):
    if len(ret_series) < 5:  # 容错：防止切片数据过短
...
def main():
    ...
    for y in years:
        ...
        if year_mask.sum() > 20:  # 确保该年有足够交易日
    ...
    for s in strats:
        ...
        df_w_temp.insert(1, '策略名称', s)  # 在 date 之后插入策略名
