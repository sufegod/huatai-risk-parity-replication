...
TEST_OUTPUT_DIR = PARAMETER_TEST_DIR / f'TL删除测试_v{VERSION}'
...
BASELINE_METRICS_PATH = BACKTEST_DIR / '回测指标' / '指标' / '年度及全局回测指标_v0.19.csv'
...
def write_comparison_outputs(metrics_filename, navs_filename, weights_filename, chart_filename):
    comparison_filename = TEST_OUTPUT_DIR / f'TL删除核心指标对比_v{VERSION}.csv'
    report_filename = TEST_OUTPUT_DIR / f'TL删除测试说明_v{VERSION}.md'

    baseline = load_total_strategy_metrics(BASELINE_METRICS_PATH, 'v0.19 IF+IC+TL')
    test = load_total_strategy_metrics(metrics_filename, 'v0.19_1 IF+IC without TL')
    ...
    if baseline is not None and test is not None:
        delta_rows = [
            {'指标': field, 'v0.19': baseline.get(field, ''), 'v0.19_1': test.get(field, ''), '差异': format_delta(field, baseline, test)}
            for field in ['年化收益', '年化波动', '夏普比率', '最大回撤', '日度胜率', '平均资金占用']
        ]
        df_delta = pd.DataFrame(delta_rows)
        comparison_note = dataframe_to_markdown(df_delta)
    else:
        comparison_note = '缺少 v0.19 或 v0.19_1 的全局指标，无法生成完整差异表。'

    report = f"""# v0.19_1 TL删除测试说明
...
# ================= 主流程 =================
def main():
    print(f"正在执行回测框架 v{VERSION}（TL删除测试，IC替换IM，日频调仓）...")
