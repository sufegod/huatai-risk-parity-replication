...
def main():
    ...
    with FILE_PATH_RETURNS.open('r', encoding='utf-8-sig') as returns_file:
        df_raw = pd.read_csv(returns_file, index_col=0, parse_dates=True)
    df_raw = df_raw.dropna(how='all')

    # 原油主连早期缺失值用布油连续替代，随后剔除布油数据
    if '原油主连' in df_raw.columns and '布油连续' in df_raw.columns:
        df_raw['原油主连'] = df_raw['原油主连'].fillna(df_raw['布油连续'])
        df_raw = df_raw.drop(columns=['布油连续'])

    df_all = df_raw.fillna(0) / 100.0

    repo_rate_ann = df_all.get('一天期国债逆回购', pd.Series(0.0, index=df_all.index))

    # ✅ 修复：通过 ASSET_CLASSES 字典生成严格的“白名单”资产池
    active_assets = []
    for class_assets in ASSET_CLASSES.values():
        active_assets.extend(class_assets)
    # 去重并确保资产确实存在于 Excel 中
    assets = [a for a in list(set(active_assets)) if a in df_all.columns]

    # 仅保留这 11 个核心资产进行回测，彻底隔离废弃数据
    df_etf = df_all[assets]

    calendar_days = df_all.index.to_series().diff().dt.days.fillna(1)
        ...
        for date, dr in next_m.iterrows():
            ...
            for s in strats:
                if date == next_m.index[0]:
                    ...
                    idle_return = idle_cash * daily_repo
                    ret_dfs[s].loc[date] = np.dot(curr_ws[s], dr.values) + idle_return

                curr_ws[s] = (curr_ws[s] * (1 + dr.values)) / (np.sum(curr_ws[s] * (1 + dr.values)) or 1)
                curr_margin[s] = np.sum(curr_ws[s] * m_ratios)
    ...
    def append_metrics(period_label, start_d, end_d):
        ...
        for asset in assets:
            m = calculate_metrics(df_etf.loc[start_d:end_d, asset], rf_slice)
            m['回测区间'] = period_label
            m['组合/资产'] = asset
    ...
    append_metrics('全局 (Total)', first_date, df_etf.index[-1])

    years = sorted(set(df_etf.loc[first_date:].index.year))
    for y in years:
        year_mask = (df_etf.index.year == y) & (df_etf.index >= first_date)
        if year_mask.sum() > 20:
            y_start = df_etf.index[year_mask][0]
            y_end = df_etf.index[year_mask][-1]
