...
本目录用于生成基于 v0.18 策略逻辑的每日回测报告。脚本运行时会先调用数据更新脚本，更新成功后再读取更新后的日涨跌幅数据，并加载 `策略复现与回测/策略代码/资产风险平价策略0.18（保证金修改+资金占用显示+日频调仓）.py` 中完整展示的回测代码，生成净值、指标、仓位明细、图表和 Markdown 报告。
...
日期后缀使用实际参与策略计算的数据日期。报告中的 `日报观察日` 始终等于策略数据日期；`仓位来源观察日` 表示当前目标仓位来自哪一个日度观察日。脚本默认保持 v0.18 的日频调仓规则：每个实际交易日生成下一交易日目标仓位，完整历史回测也沿用该日频规则。
